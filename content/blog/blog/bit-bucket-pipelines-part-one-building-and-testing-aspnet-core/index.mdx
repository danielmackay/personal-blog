...
## Summary
After completing the above we now have a Bit Bucket Continuous Integration Pipeline that builds and tests our .NET Core application on every commit.  We've seen how to change the docker image based on the type of code we're building.  And also how to trigger builds based on different condtions like pull requests or specific branches. And best of all we don't have any build servers to manage!

In my next article I'll cover how to use Bit Bucket Pipelines to package up a .NET Core application and publish it to Azure.
